<?php

/**
 * YFD AI Taxonomy v1.8 (September 2026) — closed list.
 * Category lists remain compatible with v1.3–v1.7; v1.8 clarifies field scope per jenis.
 * AI hanya boleh memilih dari daftar ini; bucket ditentukan sistem.
 *
 * @see YFD_AI_Taxonomy_v1.8.pdf
 */
return [
    'expense_categories' => [
        'Makanan & Minuman',
        'Tempat Tinggal',
        'Transportasi',
        'Komunikasi',
        'Kesehatan & Kebersihan Diri',
        'Pendidikan',
        'Investasi & Tabungan',
        'Proteksi',
        'Lifestyle & Hiburan',
        'Traveling',
        'Sosial & Keluarga',
        'Bisnis & Karir',
        'Hadiah',
        'Cicilan & Hutang',
        'Pakaian & Aksesoris',
        'Lain-lain',
        'Biaya Legal, Administrasi & Peristiwa Besar',
    ],

    'income_categories' => [
        'Gaji',
        'Bonus',
        'Freelance',
        'Affiliate',
        'Dividen',
        'Bunga Investasi',
        'Cashback',
        'Refund',
        'Penjualan',
        'Sewa Masuk',
        'Transfer Masuk',
        'Lain-lain',
    ],

    'fallback_category' => 'Lain-lain',

    /**
     * Alias lama / AI variants → kategori resmi.
     *
     * @var array<string, string>
     */
    'aliases' => [
        'gaji' => 'Gaji',
        'bonus' => 'Bonus',
        'freelance' => 'Freelance',
        'honor' => 'Freelance',
        'affiliate' => 'Affiliate',
        'afiliasi' => 'Affiliate',
        'komisi' => 'Affiliate',
        'commission' => 'Affiliate',
        'referral' => 'Affiliate',
        'dividen' => 'Dividen',
        'dividend' => 'Dividen',
        'bunga investasi' => 'Bunga Investasi',
        'bunga' => 'Bunga Investasi',
        'cashback' => 'Cashback',
        'refund' => 'Refund',
        'penjualan' => 'Penjualan',
        'sewa masuk' => 'Sewa Masuk',
        'transfer masuk' => 'Transfer Masuk',
        'makan' => 'Makanan & Minuman',
        'makanan' => 'Makanan & Minuman',
        'makanan & minuman' => 'Makanan & Minuman',
        'makanan dan minuman' => 'Makanan & Minuman',
        'jajan' => 'Makanan & Minuman',
        'minuman' => 'Makanan & Minuman',
        'transport' => 'Transportasi',
        'transportasi' => 'Transportasi',
        'listrik' => 'Tempat Tinggal',
        'air' => 'Tempat Tinggal',
        'sewa/tempat tinggal' => 'Tempat Tinggal',
        'sewa' => 'Tempat Tinggal',
        'tempat tinggal' => 'Tempat Tinggal',
        'komunikasi' => 'Komunikasi',
        'kesehatan' => 'Kesehatan & Kebersihan Diri',
        'kesehatan & kebersihan diri' => 'Kesehatan & Kebersihan Diri',
        'handbody' => 'Kesehatan & Kebersihan Diri',
        'hand body' => 'Kesehatan & Kebersihan Diri',
        'body lotion' => 'Kesehatan & Kebersihan Diri',
        'deodoran' => 'Kesehatan & Kebersihan Diri',
        'perawatan diri' => 'Kesehatan & Kebersihan Diri',
        'laundry' => 'Kesehatan & Kebersihan Diri',
        'cuci baju' => 'Kesehatan & Kebersihan Diri',
        'dry clean' => 'Kesehatan & Kebersihan Diri',
        // Perawatan & kecantikan tetap di kategori Kesehatan (bucket Flexible via Wants).
        'skincare' => 'Kesehatan & Kebersihan Diri',
        'serum' => 'Kesehatan & Kebersihan Diri',
        'make up' => 'Kesehatan & Kebersihan Diri',
        'makeup' => 'Kesehatan & Kebersihan Diri',
        'kecantikan' => 'Kesehatan & Kebersihan Diri',
        'perawatan & kecantikan' => 'Kesehatan & Kebersihan Diri',
        'dandan' => 'Kesehatan & Kebersihan Diri',
        'lipstik' => 'Kesehatan & Kebersihan Diri',
        'parfum' => 'Kesehatan & Kebersihan Diri',
        'toner' => 'Kesehatan & Kebersihan Diri',
        'facial' => 'Kesehatan & Kebersihan Diri',
        'pelembab' => 'Kesehatan & Kebersihan Diri',
        'spa' => 'Kesehatan & Kebersihan Diri',
        'minoxidil' => 'Kesehatan & Kebersihan Diri',
        'nivea' => 'Kesehatan & Kebersihan Diri',
        'sikat gigi' => 'Kesehatan & Kebersihan Diri',
        'tumbler' => 'Tempat Tinggal',
        'thumbler' => 'Tempat Tinggal',
        'termos' => 'Tempat Tinggal',
        'perabot' => 'Tempat Tinggal',
        'pendidikan' => 'Pendidikan',
        'saham' => 'Investasi & Tabungan',
        'reksadana' => 'Investasi & Tabungan',
        'emas' => 'Investasi & Tabungan',
        'obligasi' => 'Investasi & Tabungan',
        'tabungan/investasi' => 'Investasi & Tabungan',
        'investasi' => 'Investasi & Tabungan',
        'investasi & tabungan' => 'Investasi & Tabungan',
        'asuransi' => 'Proteksi',
        'proteksi' => 'Proteksi',
        'hiburan' => 'Lifestyle & Hiburan',
        'lifestyle' => 'Lifestyle & Hiburan',
        'lifestyle & hiburan' => 'Lifestyle & Hiburan',
        'subscription' => 'Lifestyle & Hiburan',
        'elektronik' => 'Lifestyle & Hiburan',
        'peralatan' => 'Lifestyle & Hiburan',
        'traveling' => 'Traveling',
        'liburan' => 'Traveling',
        'hotel' => 'Traveling',
        'staycation' => 'Traveling',
        'wisata' => 'Traveling',
        'social' => 'Sosial & Keluarga',
        'sosial' => 'Sosial & Keluarga',
        'sosial & keluarga' => 'Sosial & Keluarga',
        'qurban' => 'Sosial & Keluarga',
        'kurban' => 'Sosial & Keluarga',
        'bisnis' => 'Bisnis & Karir',
        'bisnis & karir' => 'Bisnis & Karir',
        'jasa' => 'Bisnis & Karir',
        'hadiah' => 'Hadiah',
        'tip' => 'Hadiah',
        'tips' => 'Hadiah',
        'cicilan' => 'Cicilan & Hutang',
        'cicilan & hutang' => 'Cicilan & Hutang',
        'pakaian' => 'Pakaian & Aksesoris',
        'pakaian & aksesoris' => 'Pakaian & Aksesoris',
        'pakaian dan aksesoris' => 'Pakaian & Aksesoris',
        'fashion' => 'Pakaian & Aksesoris',
        'baju' => 'Pakaian & Aksesoris',
        'sepatu' => 'Pakaian & Aksesoris',
        'tas' => 'Pakaian & Aksesoris',
        'aksesoris' => 'Pakaian & Aksesoris',
        'seragam' => 'Pakaian & Aksesoris',
        'komunikasi & administrasi' => 'Komunikasi',
        'biaya admin' => 'Komunikasi',
        'admin bank' => 'Komunikasi',
        'admin transfer' => 'Komunikasi',
        'biaya transfer' => 'Komunikasi',
        'admin atm' => 'Komunikasi',
        'pbb' => 'Tempat Tinggal',
        'pajak bumi' => 'Tempat Tinggal',
        'pajak bangunan' => 'Tempat Tinggal',
        'art' => 'Tempat Tinggal',
        'pembantu' => 'Tempat Tinggal',
        'babysitter' => 'Tempat Tinggal',
        'baby sitter' => 'Tempat Tinggal',
        'pengasuh' => 'Tempat Tinggal',
        'sopir pribadi' => 'Tempat Tinggal',
        'driver pribadi' => 'Tempat Tinggal',
        'fisioterapi' => 'Kesehatan & Kebersihan Diri',
        'rehab' => 'Kesehatan & Kebersihan Diri',
        'rehabilitasi' => 'Kesehatan & Kebersihan Diri',
        'seminar' => 'Pendidikan',
        'workshop' => 'Pendidikan',
        'sertifikasi' => 'Pendidikan',
        'conference' => 'Pendidikan',
        'pengembangan diri' => 'Pendidikan',
        'iuran organisasi' => 'Pendidikan',
        'idi' => 'Pendidikan',
        'stnk' => 'Cicilan & Hutang',
        'pkb' => 'Cicilan & Hutang',
        'samsat' => 'Cicilan & Hutang',
        'pajak kendaraan' => 'Cicilan & Hutang',
        'notaris' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'balik nama' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'ajb' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'mahar' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'pernikahan' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'duka' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'pemakaman' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'biaya legal' => 'Biaya Legal, Administrasi & Peristiwa Besar',
        'lainnya' => 'Lain-lain',
        'lain-lain' => 'Lain-lain',
        'lain lain' => 'Lain-lain',
    ],

    'source_of_truth_note' => [
        'Taxonomy tertutup (YFD AI Taxonomy v1.8): AI HANYA memilih dari 17 kategori resmi (+ kategori pemasukan).',
        'AI tidak boleh membuat kategori baru. Jika ragu → Lain-lain (< 2% target).',
        'Layer 1 = Kategori (closed list). Layer 2 = Bucket (otomatis dari mapping sistem).',
        'Jenis transaksi: Pemasukan | Pengeluaran | Saving/Investment | Kewajiban Pajak | Piutang Keluar | Piutang Masuk | Utang Masuk | Utang Keluar.',
        'Kewajiban Pajak & Likuiditas Sosial (Piutang + Hutang) dikecualikan dari 4 bucket.',
        'Utang Masuk = terima pinjaman sosial (bukan Pemasukan). "saya pinjam uang [nama]" tanpa ke/dari tetap Utang Masuk.',
        'Utang Keluar = bayar balik (bukan Pengeluaran prescription). Piutang Keluar = pinjamin/talangin; Piutang Masuk = orang lain mengembalikan.',
        'Tujuan pinjaman (kuliah/obat/RS) dan jatuh tempo (kembali bulan depan) bukan jenis transaksi.',
        'PRINSIP: Likuiditas Sosial hanya mengubah kas & aset/kewajiban sementara. Belanja dari dana pinjaman tetap dicatat Pengeluaran + bucket (transaksi terpisah).',
        'PBB → Tempat Tinggal (bucket ikut fungsi properti). Pajak kendaraan (STNK/PKB) → Cicilan & Hutang.',
        'Notaris/mahar/duka → Biaya Legal, Administrasi & Peristiwa Besar.',
        'Admin bank / biaya transfer → Komunikasi (bukan Lain-lain / bukan Legal Administrasi).',
        'Gym/olahraga berbayar → Lifestyle & Hiburan / Flexible + Social / Wants (bukan Essential).',
        'Laundry/cuci baju → Kesehatan & Kebersihan Diri / Essential Living.',
        'Fashion/baju/sepatu → Pakaian & Aksesoris (bukan Lifestyle).',
        'Pengembangan diri / iuran organisasi → Pendidikan / Future Building. Makan+meeting kerja → Bisnis & Karir / Future Building.',
        'ART/driver/babysitter → Tempat Tinggal (grey area sifat). Qurban → Sosial & Keluarga. Fisioterapi resep → Kesehatan Essential.',
        'Traveling: liburan/hotel/staycation/wisata. Fashion → Pakaian & Aksesoris (bukan Lifestyle).',
        'DP rumah/properti → Saving/Investment / Future Building. DP kendaraan kebutuhan sama; DP kendaraan lifestyle → Pengeluaran / Flexible + Social.',
        'Pajak kendaraan STNK/PKB → Cicilan & Hutang (bucket ikut fungsi kendaraan). Pinjol = grey area (klarifikasi AI). Denda non-pajak → Essential Living.',
        'Donasi/sedekah/zakat/qurban/hadiah → Sosial & Keluarga atau Hadiah / Flexible + Social.',
    ],
];
