@php
    ...
    $diagnosticUrl = $diagnosticUrl ?? ($portalDiagnosticUrl ?? route('portal.diagnostic'));
    $compact = $compact ?? false;
    $skipActivation = $skipActivation ?? false;
@endphp
<div class="rounded-2xl border-2 border-gold-400/60 bg-gradient-to-br from-navy-800 to-navy-700 text-white shadow-lg overflow-hidden mb-6">
    ...
    <ol class="p-5 sm:p-6 space-y-4 {{ $compact ? 'text-sm' : '' }}">
        ...
        <li class="flex gap-3">
            <span class="w-7 h-7 shrink-0 rounded-full bg-white/15 flex items-center justify-center text-xs font-bold text-gold-400">{{ $skipActivation ? '2' : '3' }}</span>
            <div>
                <div class="font-semibold">Catat transaksi lewat YFD First Aid</div>
                <p class="text-white/75 text-sm mt-0.5">Kirim catatan harian di Telegram (contoh: <em>kopi 25rb</em>) — mood & deteksi impulsif ikut tercatat.</p>
            </div>
        </li>
        <li class="flex gap-3">
            <span class="w-7 h-7 shrink-0 rounded-full bg-white/15 flex items-center justify-center text-xs font-bold text-gold-400">{{ $skipActivation ? '3' : '4' }}</span>
            <div>
                <div class="font-semibold">Pantau dashboard</div>
                <p class="text-white/75 text-sm mt-0.5">Financial Health Dashboard & Behavioral Dashboard terisi otomatis setelah diagnostik + transaksi.</p>
            </div>
        </li>
    </ol>
</div>
